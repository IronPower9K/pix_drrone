...
try:
    ...
    while True:
        ...
        img = 255 - img
        hsv = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)    # BGR을 HSV로 변환해줌
        ...
        res = cv2.bitwise_and(img, img, mask=mask)      # 흰색 영역에 파랑색 마스크를 씌워줌.
       
        blue_x = np.where(res>10)
       
        

        
        if len(blue_x[0]) > 0:
            track_bluey = np.mean(blue_x[0])
            track_bluex = np.mean(blue_x[1])
            count_zero = 0
            for arr in blue_x:
            ...
            if track_bluex >= 280:
                ...
                print('right')
            elif track_bluex <= 220:
                
                print('left')
                
            elif track_bluey >= 300:
                print('down')
                #me.move_down(20)
            elif track_bluey <= 60:
                print('up')
            ...
            set_attitude(yaw_rate=10, thrust=0.5, duration=1)



        cv2.imshow('img',img)       # 원본 영상을 보여줌
        cv2.imshow('tracking', res)           # 마스크 위에 파랑색을 씌운 것을 보여줌.
